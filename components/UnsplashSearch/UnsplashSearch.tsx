...
> {
  ...
  componentDidMount() {
    this.queryPhotos("scenic nature");
  }

  async queryPhotos(query: string, page: number = 1) {
    this.setState({ querying: page === 1 });
    const { accessKey, photosPerPage } = this.props;
    const url = `https://api.unsplash.com/search/photos?client_id=${"Gh7x3qW_v2v6kdbTaPLShzIMCyyTIP5At-4J98rqjLg"}&query=${query}&page=${page}&per_page=${photosPerPage}&orientation=landscape`;
